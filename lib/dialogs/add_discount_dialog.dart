...
class AddToDiscount extends StatefulWidget{
  final List<SchemesDataDetail> discountList;
  final int selectedUser;
  final List<ProductDataDetail> items;
  final SharedPreferences prefs;
  final Function addSchemeToCart;

  const AddToDiscount({Key? key, required this.discountList, required this.selectedUser, required this.items, required this.prefs, required this.addSchemeToCart}) : super(key: key);

  @override
  State<AddToDiscount> createState()=> _AddToDiscount();

}

class _AddToDiscount extends State<AddToDiscount>{
      ...
      child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 15,),
              Text(AppStrings.discountOffer,style: TextStyle(color: ColorConstants.colorPrimary,fontSize: 12,fontWeight: FontWeight.w500),),
              SizedBox(height: 5,),
              Container(
                child: getDiscountView(),
              ),
              SizedBox(height: 15,),
              Container(
  ...
  void setProducts() async {
    List<SchemesDataDetail> schemeRows = widget.discountList;
    var disc = 0;
  ...
  double getDiscount() {
    var disc = 0.0;
    if(schemes[0].discountUom=='%'){
      double price = double.parse(getPriceForNonFreeItems());
      disc = (price*100)/(double.parse(schemes[0].discountUom!));
    }
    if(schemes[0].discountUom=='INR'){
      int freeFactor = AppUtils.getFreeFactor(items, schemes);
      disc = schemes[0].discountValue!*freeFactor;
    }
    return disc;
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
              flex: 2,
              child: Text(AppStrings.get,style: TextStyle(color: Colors.grey,fontSize: 12),)
          ),
          Expanded(
              flex: 8,
              child: Text('${AppStrings.discountOf}  ${getDiscountText()}',style: TextStyle(color: Colors.black,fontSize: 12,fontWeight: FontWeight.w500))
          )
        ],
      ),
    );
  ...
  getFreeFactor() {
    List<int> freeList = [];
    for(var item in items)  {
      for(var scheme in (widget.discountList.where((element) => !element.isQps!).toList())){
  ...
  addSchemeToCart() async{
    if(await AppUtils.checkAllItemsHaveDistTypes(items,widget.selectedUser,instance)){
      if(await AppUtils.hasCommonDistType(items,widget.selectedUser,instance)){
        ...
        widget.addSchemeToCart(items,freeItem,discountId,distributorTypeIds,distributorTypes);
  ...
  getDiscountText() {
    if(schemes[0].discountUom=='INR'){
      return '${AppUtils.getCurrency(widget.prefs)}${schemes[0].discountValue}';
    }
    if(schemes[0].discountUom=='%'){
      return '${schemes[0].discountValue} ${schemes[0].discountUom}';
    }
    return '';
  }
